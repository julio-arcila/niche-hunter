...
def criteria(
    report: bool = typer.Option(
        False, "--report", help="Write reports/production_criteria_<date>.md"
    ),
) -> None:
    """The eight production criteria, graded from the artefacts.

    Exits non-zero while any is unmet, so "production grade" is a command rather than a
    memory. Two of the eight (unattended nights, fixture freshness) are satisfied by
    elapsed time — this is what renders that honestly instead of leaving a slice reading
    open for a month.
    """
    from datetime import date as _date

    from nh.jobs.criteria import evaluate

    results = evaluate()
    typer.echo(f"{'':<9}{'#':<3}{'CRITERION':<16}DETAIL")
    for r in results:
        colour = {True: typer.colors.GREEN, False: typer.colors.YELLOW, None: None}[r.met]
        typer.secho(f"{r.label:<9}C{r.n:<2}{r.name:<16}{r.detail}", fg=colour)
        typer.echo(f"{'':<9}   {'':<16}evidence: {r.evidence}")

    met = sum(1 for r in results if r.met)
    typer.echo(f"\n{met}/{len(results)} met")

    if report:
        path = Path("reports") / f"production_criteria_{_date.today()}.md"
        lines = [
            f"# Production criteria — {_date.today()}",
            "",
            f"**{met} of {len(results)} met.** Generated by `nh criteria --report`; every row is",
        ...
        lines += [
            f"| {r.n} | {r.name} | {r.label} | {r.detail} | `{r.evidence}` |" for r in results
        ]
        path.write_text("\n".join(lines) + "\n")
        typer.echo(f"wrote {path}")

    raise typer.Exit(0 if met == len(results) else 1)
